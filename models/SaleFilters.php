...
class SaleFilters extends \yii\db\ActiveRecord
{
    ...
    public function reset()
    {
        ...
        info("SALEFILTER LIST WAS " . $this->count, DANGER);
        $this->white_list_id = Methods::convertToStringWithBorders(Sale::find()->select('id')->where(['in', 'id', Methods::convertToArrayWithBorders($this->white_list_id)])->column());
        info("SALEFILTER LIST NOW " . $this->count, SUCCESS);


        $this->black_list_id = Methods::convertToStringWithBorders(Sale::find()->select('id')->where(['in', 'id', Methods::convertToArrayWithBorders($this->black_list_id)])->column());
    }
    ...
    function rules()
    {
        return [
            ...
            [['name', 'locality', 'tags_id', 'photo', 'regions', 'plus_tags','image'], 'string', 'max' => 255],
    ...
    function getCount()
    {
        return count(Methods::convertToArrayWithBorders($this->white_list_id));
