...
class Synchronization extends Sale
{
    ...
    public function SalefiltersCheck()
    {
        ...
        info("COUNT FILTERS TO INFORM " . count($salefilters));
        if ($salefilters) {
            foreach ($salefilters as $salefilter) {
                info(" SALEFILTER_NAME = " . $salefilter->name, SUCCESS);

                if ($response = $salefilter->Is_in_salefilter($this)) {
                    info(" SALE IS IN SALE FILTER ", SUCCESS);
                    $message = $this->inform_message;
                    $salefilter->notify($message, $response);
    ...
    public
    static function getActiveItemFromBase($parsing)
    {
        // поиск, что данный id уже есть в базе
        $active_item = Synchronization::find()
    ...
    public function renderProcessingStatuses()
    {
        return " STATUS: " . \app\models\Sale::DISACTIVE_CONDITIONS_ARRAY[$this->disactive] .
            " <hr style=\"margin-top: 0px; margin-bottom: 0px;\">parsed: " . \app\models\Sale::TYPE_OF_PARSED[$this->parsed] .
            " <hr style=\"margin-top: 0px; margin-bottom: 0px;\">geocodated: " . \app\models\Geocodetion::GEOCODATED_STATUS_ARRAY[$this->geocodated] .
            " <hr style=\"margin-top: 0px; margin-bottom: 0px;\">processed: " . \app\models\Sale::TYPE_OF_PROCCESSED[$this->processed] .
            " <hr style=\"margin-top: 0px; margin-bottom: 0px;\">load_analized: " . \app\models\Sale::TYPE_OF_ANALIZED[$this->load_analized] .
            " <hr style=\"margin-top: 0px; margin-bottom: 0px;\">sync: " . \app\models\Sale::TYPE_OF_SYNC[$this->sync];


    }

    public
    static function getExistedItem($parsing, $config)
    {
    ...
    static function TODO_NEW($parsing, $config, $options = [])
    {
        ...
        if ($active_item) {
            ...
            $active_item = New Synchronization();
            $active_item->id_category = $config->id;
            $active_item->loadDefaultStatuses();
            ...
            // echo "<br>" . $parsing['url'] . " новый объект";
              ...
              if (!$active_item->save()) my_var_dump($active_item->getErrors());
              $active_item->setProccessingLog(Sale::NEW_ITEM);
    ...
    public
    static function getCachedCategory($id_category)
    {
...
//    }
    ...
    */
    ...
    function checkForAgents()
    {
        ...
        $agent = Agents::find()->where(['phone' => $this->phone1])->andWhere(['person_type' => Agents::PERSON_TYPE_AGENT])->one();
        if ($agent) {
