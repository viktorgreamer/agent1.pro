...
<div class="tags-quick-add-form">
                ...
                <div class="card-body">
                    ...
                    if (count($tags) != 0) {
                        foreach ($tags as $tag) {
                            ...
                            <a href="#" class="tags-action-button-salelist"
                               data-salelist_id="<?= $id ?>"
                               data-tag_id="<?= $tag->id ?>">  <span class="badge badge-<?= $tag->color; ?> <? if (!in_array($tag->id, $Realtags)) echo "half-opacity"; ?>">
                              <?= $tag->name; ?></span></a>
    ...
    ?>
    <hr>
    <a href="<?= Url::toRoute(['/tags/create']) ?>" target="_blank"> +new tag</a><button type="button" class="btn btn-success btn-sm" data-dismiss="modal" aria-label="Close"
                                                                                         id="closeModal">
        Закрыть
    </button>
